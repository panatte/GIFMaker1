...
function Navbar() {
    ...
    const getProfile = async () => {
        ...
        fetch("/api/Checkcookies", {
            ...
            .then((data) => {
                if (data.status === 200) {
                    if (data.data.path_profile !== 'null' && data.data.path_profile !== null) {
                ...
                <div className="items-center justify-between hidden w-full md:flex md:w-auto md:order-1" id="navbar-cta">
                    <ul className="flex flex-col font-medium p-4 md:p-0 mt-4 border border-gray-100 rounded-lg md:space-x-8 rtl:space-x-reverse md:flex-row md:mt-0 md:border-0 dark:bg-gray-800 md:dark:bg-gray-900 dark:border-gray-700">
                        <li className="bg-indigo-900">
                            <Link href="/admin" className={`text-2xl font-semibold block py-2 px-3 md:p-0 rounded hover:bg-fuchsia-500 md:hover:bg-transparent md:hover:text-fuchsia-500 md:dark:hover:text-fuchsia-300 dark:border-fuchsia-300 ${isActive('/admin')}`}>Manage users</Link>
                        </li>
                        <li className="bg-indigo-900">
